...
document.addEventListener('DOMContentLoaded', async () => {
        ...
        switch (contentData.type) {
          case 'series': {
            if (startFromBeginning || activeWatchData.status !== 'in_progress')
              loadEpisode(contentData.episodes[0], false);
            else {
              ...
              loadEpisode(epIndex >= 0 ? epIndex : 0, false);
              renderEpisodes();
  ...
  function loadEpisode(index, autoplay = false) {
    const ep = contentData.episodes.filter(ep => ep._id == index._id)[0];
    if (!ep) return;
    currentIndex = index;
    // עדכון UI
