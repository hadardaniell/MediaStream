...
export const createUser = async (req, res) => {
    try {
        ...
        const passowrdHash = password ? await bcrypt.hash(password, 10) : undefined;
        const user = await UsersModel.create({email, passowrdHash, name, roles});
...
export const updateUser = async (req, res) => {
    try {
        ...
        if (patch.password) {
            patch.passowrdHash = await bcrypt.hash(patch.password, 10);
